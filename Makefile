...
ifeq (, $(PKG_CONFIG))
  ...
  ifneq (0, $(shell $(PKG_CONFIG) --exists ncursesw ; echo $$?))
...
ifneq (0, $(NO_PKG_CONFIG_NCURSES))
  ...
  CURSES_CFLAGS = $(shell $(PKG_CONFIG) --cflags ncursesw)
  CURSES_LIBDIRS = $(shell $(PKG_CONFIG) --libs-only-L ncursesw)
  CURSES_LIBS = $(shell $(PKG_CONFIG) --libs-only-l --libs-only-other ncursesw)
endif
